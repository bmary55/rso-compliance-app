...
export async function POST() {
  try {
    ...
    // Insert new sample patients with more scan types
    const patients = [
    ...
    client.release();
    return NextResponse.json({ success: true, message: 'Data reseeded successfully' });
  } catch (error) {
    console.error('Reseed error:', error);
    return NextResponse.json({ error: 'Failed to reseed data' }, { status: 500 });
  }
}
